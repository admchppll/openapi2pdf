...
export function getInfoDef(spec, bookTitle, localize) {
  ...
  if (spec.info) {
      ...
      if (spec.info.contact.url) {
        contactUrl = {
          text: [
            ...
            { text: spec.info.contact.url, style: ['small', 'blue'], link: spec.info.contact.url },
          ],
        };
      }
      if (spec.info.termsOfService) {
        termsOfService = {
          text: [
            ...
            { text: spec.info.termsOfService, style: ['small', 'blue'], link: spec.info.termsOfService },
          ],
        };
      }
      contactDef = [
    ...
    content = [
      ...
      spec.info.version
        ? { text: `${localize.apiVersion}: ${spec.info.version}`, style: ['p', 'b', 'right', 'alternate'] }
        : '',
      specInfDescrMarkDef,
      ...contactDef,
    ...
    for (const key in spec.securitySchemes) {
      tableContent.push([
        ...
        spec.securitySchemes[key].type +
          (spec.securitySchemes[key].scheme ? `, ${spec.securitySchemes[key].scheme}` : '') +
          (spec.securitySchemes[key].bearerFormat ? `, ${spec.securitySchemes[key].bearerFormat}` : ''),
        ...
        },
        {
          stack: [
            ...
            paramSchema.allowedValues
              ...
              : '',
            paramSchema.pattern
              ? { text: `${localize.pattern}: ${paramSchema.pattern}`, style: ['small', 'gray'] }
              : '',
          ],
        },
        {
      ...
      if (origSchema) {
        ...
        if (schemaStyle === 'object') {
          ...
          if (schemaInObjectNotaion['::type'] && schemaInObjectNotaion['::type'] === 'array') {
            ...
            rootObjectType = [
              { text: 'ARRAY OF OBJECT WITH BELOW STRUCTURE', style: ['sub', 'b', 'alternate'], colSpan: 3 },
            ];
          } else {
            schemaTableTreeDef = objectToTableTree(schemaInObjectNotaion, localize);
