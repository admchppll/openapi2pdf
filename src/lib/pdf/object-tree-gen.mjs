...
export function getTypeInfo(schema) {
  ...
  const info = {
    type: schema.$ref ? '{recursive}' : schema.enum ? 'enum' : schema.format ? schema.format : schema.type,
    format: schema.format ? schema.format : '',
    pattern: schema.pattern && !schema.enum ? schema.pattern : '',
    readOrWriteOnly: schema.readOnly ? 'READ-ONLY' : schema.writeOnly ? 'WRITE-ONLY' : '',
  ...
  if (schema.type === 'array' || schema.items) {
    ...
    if (schema.minimum !== undefined && schema.maximum !== undefined) {
      info.constrain = `${schema.exclusiveMinimum ? '>' : 'between '}${schema.minimum} and ${
...
export function objectToTree(obj, localize, prevKeyDataType = 'object', prevKey = '') {
  ...
  if (prevKey.startsWith('OPTION:')) {
    ...
    keyDef = {
      stack: [{ text: `${prevKey} ${prevKeyDataType === 'array' ? '[{' : '{'}`, style: ['small', 'mono'] }],
    };
